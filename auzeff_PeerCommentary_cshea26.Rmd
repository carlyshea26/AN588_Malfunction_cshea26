...
Z.prop.test <- function(p1, n1, p2 = NULL, n2 = NULL, p0, alternative = "two.sided", conf.level = 0.95) {
  if (is.null(p2) || is.null(n2)) {
    ...
    if ((n1 * p0 < 5) || (n1 * (1-p0) < 5)) {
      ...
      # AZ: I think that these were supposed to be "< 5", but you accidentally wrote "> 5" instead.
    ...
    if ((n1 * p0 < 5) | (n1 * (1 - p0) < 5 ) | (n2 * p0 < 5) | (n2 * (1 - p0) < 5)) {
      ...
      # AZ: I could be wrong, but I think that these shouldn't use the p0 value since it's a two-sample test.  Apart from that, this is a much more efficient way of returning the warnings than what I did!
  ...
  ci <- c(lower, upper)
    ...
    result2 <- list(test.type = "Two-Sample Proportion Z-test", 
      ...
      # AZ: I like that yoou returned the test type and the critical value as part of the list--I didn't think to do that!
...
> AZ: Overall, this function looks good to me!  Our tests seem to be returning the same values for most of the tests, although to be fair I'm not confident enough in my own function to say that that definitely means it's right.  The one situation where we seem to be getting different results in in the two-sample two-tail test; specifically, our p-values are different.  Your p-value in this situation is higher than 1.  To be honest, I have no idea why ours are different; it doesn't seem to be a problem in any of the other two-sample tests.
