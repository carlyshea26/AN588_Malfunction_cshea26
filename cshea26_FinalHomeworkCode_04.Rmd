...
g <- g + geom_smooth(method = "lm", formula= y~x, color = "forestgreen") # makes the regression line
g <- g + geom_text(x = 400, y = 300, label = mod1, color = "forestgreen") #x and y here center my equation wherever i tell it to
g
```
...
model2 <- lm(log(MaxLongevity_m) ~ log(Brain_Size_Species_Mean), data = data)
mod2 <- equation1(model2) #running into an error here where its giving me the equation for my model 1
...
b <- b + geom_point() + geom_smooth(method = "lm", color = "#7db569") + geom_text(x = 5, y = 5, label = mod2, color = "#7db569")
b
