...
For model 2: 
```{r}
#making log transformed variables
...
# add confidence/prediction intervals to the plot 
lines(v_log, ci[, 1], col = "black")
lines(v_log, pi[, 2], col = "red")
lines(v_log, pi[, 3], col = "red")  

#alternative way to create a plot with ggplot
...
# plot alternative plot b using ggplot
b <- ggplot(data, aes(x = log_Brain_Size, y = log_Longevity)) +
  geom_point() +
...
b
```
#Part 3 Alternative: With Prediction Lines and CIs
...
a <- ggplot(datas, aes(x = Brain_Size_Species_Mean, y = MaxLongevity_m)) +
  ...
  theme_minimal()
a <- a + geom_text(x = 350, y = 250, label = mod1, color = "blue")

# adding my regression line (black)
a <- a + geom_line(aes(y = CIfit), size = 1)

# then add 90% confidence intervals (dotted red lines)
a <- a + geom_line(aes(y = CIlwr), linetype = "dotted", color = "90% CI", size = 1) + 
  geom_line(aes(y = CIupr), linetype = "dotted", color = "90% CI", size = 1)

# then add prediction intervals (blue)
a <- a + geom_line(aes(y = PIlwr), color = "90% PI", size = 1) + 
  geom_line(aes(y = PIupr), color = "90% PI", size = 1)
...
a
a <- a + scale_color_manual(values = c("Data Points" = "black", 
...
#for model2 (log)
ci <- predict(model2, interval = "confidence", level = 0.90)
pi <- predict(model2, interval = "prediction", level = 0.90)
```
