...
idek of this is correct? why are my lines curving for confidence interval?
```{r}
m <- lm(longevity ~ Brain_Size_Species_Mean, data = monkey_clean)
summary(m)
ci1 <- confint(m, level = 0.90); ci1
ci2 <- predict(m, newdata = data.frame(Brain_Size_Species_Mean = monkey_clean$Brain_Size_Species_Mean), interval = "confidence",
    level = 0.90) #max suggested this and the lines variable to make the ci and slope lines
head(ci2)
lines <- data.frame(ci2)

#trying to make a plot for confidence intervals
model_graph<- ggplot(monkey_clean, aes(x = brain, y = longevity))   + 
  geom_point() +    
  geom_line(aes(x = monkey_clean$Brain_Size_Species_Mean, y = lines$fit), col = "blue") +
  geom_line(aes(x = monkey_clean$Brain_Size_Species_Mean, y = lines$upr), col = "red") +
  geom_line(aes(x = monkey_clean$Brain_Size_Species_Mean, y = lines$lwr), col = "red") +
...
p1 <- predict(model, newdata = data.frame(brain = 800), interval = "prediction", level = 0.90)
p1
