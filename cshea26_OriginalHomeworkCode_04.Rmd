...
library(curl)
library(ggplot2)
library(ggpubr) #this allows me to display a fancy equation on my linear regressin plot
f <- curl("https://raw.githubusercontent.com/fuzzyatelin/fuzzyatelin.github.io/master/AN588_Fall21/KamilarAndCooperData.csv")
monkey <- read.csv(f, header = TRUE, sep = ",", stringsAsFactors = FALSE)
head(monkey)
```
I am going to use this format from module 12 to accomplish creating a linear regression: 
...
g
```{r}
#we can now isolate the columns that we want to look at exclusively
#need to clean my data first to remove unecessary info, or else i get a warning sign when i try to plot

monkey_clean <- na.omit(monkey[, c("MaxLongevity_m", "Brain_Size_Species_Mean")])
longevity <- monkey_clean$MaxLongevity_m #extracting clean variables
brain <- monkey_clean$Brain_Size_Species_Mean

y <- brain_size - mean(brain) #here could also just use monkey$MaxLongevity_m instead of brain_size
x <- longevity - mean(longevity)
z <- data.frame(cbind(x, y)) #this creates a data frame that i can then use
g <- ggplot(data = monkey, aes(x = longevity, y = brain)) + geom_point()
